...
The fourth result shows the limit of disclosure, and it required an experiment
rather than a re-reading. The offset study attaches its reversal to a single
condition because its grid steps from zero to \CoarseGapHigh{} milliseconds in
one move and has nothing in between. We reran
...
the conditions a study ran can recover the conditions it did not. One more
condition is what closes that gap, since the trials a disclosure could release
were never generated, and the record examined here contains enough to say where
to put it.

Two things in this paper are offered for reuse. The first is an auditable
stopping-point protocol: a monotone ladder whose reportable quantity is the
index of the first break, a read-only probe that re-checks each rung on the live
machine and records the observation with a timestamp, and two off-ladder axes,
corpus presence and per-trial disclosure, recorded beside the index rather than
folded into it. Applied here, the protocol locates the first break at rung
\FirstBreak{} on one machine on one date and records the observation that
justifies it. The second is a paired-replication design for the condition a
sweep skipped: the same estimator code on a grid refined inside the skipped
interval, crossed with independently simulated arrays, every trial seeded from
its own cell so that a condition added later leaves the recorded cells
untouched, one prediction and its tolerance fixed before the first cell ran, and
every trial stored. Its registered result is the crossing located on
\FineTrials{} paired trials per cell, reported as an interval across the arrays
that crossed, with the arrays that had not crossed by the top of the grid
...
Proposition~\ref{prop:disclosure} states the gap, and the third comparison below
realises it: at a condition where every trial completed in both arms, the mean
and the median of the same \CxTrialsPerCondition{} pairs name different
estimators.
...
estimator, and the efficiency analysis reported below shows what it costs on this
record: the same trials against the same bound read as \CxEffMeanLow{} to
\CxEffMeanHigh{} times the bound through the mean and as \CxEffMedianLow{} to
\CxEffMedianHigh{} times it through the median.
...
gelman2014garden,simmons2011false,amrhein2019retire}. The record examined here
is a different case: it declares its display rule in a field and names the
median primary. The problem we describe survives an honest declaration, because
the reader of a published table does not receive the trials the declaration was
applied to.
...
appendix~\cite{vandewalle2009repro}. The ladder below asks a question prior to
any evaluation of those estimators: how far a reader on an ordinary machine gets
toward the table those papers would have the reader recompute.
...
Rung one holds: the audited comparison has a citable record, the {IROS} 2024
paper~\cite{hybridtdoa2024}.
...
points are on disk. The availability of this work is complete by the standard a
badge applies, and an artifact badge asserting availability would be accurate.

The first break is at rung \FirstBreak{}. The source is written for a commercial
numerical computing environment that is absent from this machine, and the free
interpreter most often proposed in its place is absent as well. The second fact
is recorded for completeness, because the methods stipulate that a substitute
...
to be established rather than assumed. Rung \NRungs{} is therefore unreachable on
this machine, and the audit's outcome is the integer \FirstBreak{} together with
the observation that justifies it.

The project's own recorded state agrees, and it records \BlockedTiers{} distinct
blocked comparisons for as many unrelated reasons: one blocked by the missing
runtime, one blocked by a dependency on both that runtime and the absent corpus,
and one blocked because the simulation retains only derived timing measurements
and no waveforms, so a whole family of signal-domain baselines is out of reach
from the stored data. That third blocker is instructive precisely because it has
nothing to do with licensing. Reachability breaks for mundane, various and
specific reasons, which is the argument for recording the specific reason rather
than a verdict.
...
A public acoustic-tracking corpus used in this line of work~\cite{locata} is
deposited in a public archive with a permanent identifier and is roughly
\CorpusGB{} gigabytes.
The probe
...
\subsection{Why the reversal happens}

The mechanism is simple, and that is what makes it worth reporting. The
offset-aware estimator always estimates the per-channel offsets, because it has
no way to know they are zero. Estimating parameters that carry no signal costs
...
Read that way, the comparison is evidence that the two estimators have different
failure modes rather than that one of them is better, and that the experiment
which distinguishes them is the one at the boundary of the modelled
...
\section{A comparison that runs, read three ways}
\label{sec:display}

\subsection{Three summaries, two answers}

Everything so far has been about whether a reader can get a comparison to run.
The third record in this project is one they can run, and it is the case that
asks what a reader holds once the code has run. It puts two locally written
estimators for the same calibration problem against each other at
\CxConditions{} timing-noise levels, \CxTrialsPerCondition{} paired trials each,
and it stores both the cell summary a paper would print and the \CxPlanned{}
individual trials that summary came from. That second half is what makes the
following possible at all.
...
\CxTrialsPerCondition{} trials at $p = \CxCleanP{}$. Two sentences, both true,
both computed from the same \CxTrialsPerCondition{} numbers, and they name
different winners.

\subsection{One trial in \CxTrialsPerCondition{}}
...
Setting trials aside here measures leverage; every quantity reported as a result
is computed from the full sample. The point is the leverage: a conclusion that
one trial in \CxTrialsPerCondition{} can reverse is a different kind of object
from a conclusion that survives, rather than a weaker version of it, and the
printed row does not distinguish them.
...
magnitude below the evaluation corpus that is absent from this machine.
...
\small
\input{generated_table_complement}
\end{table}
...
\label{tab:influence}
\small
\input{generated_table_influence}
\end{table}
...
\NTrials{}-trial extension measures \MeasuredCrossing{} milliseconds --- a
point estimate \PredictionError{} milliseconds from that prediction, inside
the \PredictionTolerance{} millisecond tolerance we agreed to in advance.

The agreement is a single point hit, on the array the original sweep ran, at
\NTrials{} trials per cell; how precisely the crossing is located is a separate
question from whether the prediction landed inside the tolerance. What the hit
shows is that the information needed to place the
missing condition was cheap and already present: it is derivable from the
numbers the record prints. A reader holding the coarse table could have located
a candidate condition whose absence made the headline read the way it does.
Running it is the step that requires a new experiment, because
Corollary~\ref{cor:onecondition} asks for trials at a condition nobody ran, and
neither the summary, the per-trial record nor the code contains those.

The scope of the agreement is that one array. The prediction succeeds for the
array the original sweep used; \CensoredRooms{} of the other arrays have no
measured crossing to test it against, so the rule is shown for that array and
left open for arrays in general. The coarse record locates its own missing
condition, and that is the extent of what the test establishes.
...
\label{tab:breakeven}
\small
\input{generated_table_breakeven}
\end{table}
...
\subsection{A \FineFourHundredTrials{}-trial re-evaluation, reported as a sensitivity}
\label{sec:fine400}

The crossings above are the registered quantity of this section, located on
\FineFourHundredRegisteredTrials{} paired trials per cell, and they remain so.
After that record was closed, the same \FineFourHundredCells{} cells --- the
same \FineRooms{} arrays, the same \FineLevels{} dispersions, the same estimator
code, the same prior width and noise floor --- were re-evaluated to
\FineFourHundredTrials{} paired trials each. Because every trial's seed is a
digest of the design, the array, the dispersion and the trial index, the first
\FineFourHundredRegisteredTrials{} trials of every cell in the larger run are
the registered draw itself, not a re-draw of it; the build checks that they are
identical to the bound trials, and that their medians reproduce the registered
per-cell medians exactly, before it reads anything else from the record.
Fig.~\ref{fig:fine400} sets the two side by side.

Read on \FineFourHundredTrials{} trials under the same crossing rule, the
ordering changes sign inside the grid in \CrossingFourHundredCrossed{} of the
\FineRooms{} arrays, between \CrossingFourHundredLo{} and
\CrossingFourHundredHi{} milliseconds; \CrossingFourHundredInside{} of those
positions fall inside the registered interval of \CrossingLow{} to
\CrossingHigh{} milliseconds, and \CrossingFourHundredCensored{} arrays are
censored under that rule. In the array the coarse sweep was run on, the
\FineFourHundredTrials{}-trial position is \CrossingFourHundredRecorded{}
milliseconds, \CrossingFourHundredRecordedGap{} milliseconds from the
prediction of \PredictedCrossing{} milliseconds. That distance is reported for
completeness and is not a second test of the prediction: the pre-agreed
comparison was against the \FineFourHundredRegisteredTrials{}-trial draw and
is settled above.

The \FineFourHundredTrials{}-trial values are a sensitivity of the located
position to the number of trials per cell. They were not pre-registered, they
were computed after the registered record was closed, and they do not replace
it. The registered quantity of this section remains the
\FineFourHundredRegisteredTrials{}-trial crossing, and every statement above
about where the sign changes refers to that record. What the re-evaluation adds
is a measure of how far a crossing located on
\FineFourHundredRegisteredTrials{} trials per point can move when the same
cells are read on \FineFourHundredRatio{} times as many, which is a fact about
the resolution of the registered grid rather than a correction to it.

\begin{figure}[tbp]
\centering
\includegraphics[width=0.88\linewidth]{fig13_fine400.pdf}
\caption{\CapFineFourHundred}
\label{fig:fine400}
\end{figure}
...
falsifiable shape: that on a stated machine on a stated date the first break was
at a stated rung for a stated reason, which is a different and smaller statement
than a verdict on the work. Anyone with a different machine can produce a
different integer, and the disagreement is then about an observation rather than
about a judgement.

Below the runtime rung the audited work is complete by everything this
instrument checks: the record is citable, the source is at its recorded revision
and every entry point is present. The runtime requirement is a property of the
...
choice. Recording the break attributes no fault. What it gives a reader deciding
whether to attempt a reproduction is a cheap way to learn that a licence stands
between them and rung \NRungs{}, which the badge on the artifact would leave
unsaid.
...
placed at an unobserved point where the sign is negative. The refined sweep adds
that the record examined here already held the position: a point estimate was
computable from its own printed medians, on the \NTrials{}-trial extension and
inside the stated \PredictionTolerance{} millisecond tolerance, by anyone who
thought to ask.

The point about summaries is the one that changes what we would build next. A
ladder whose top rung is ``the published table recomputes'' treats reproduction
as the end of the road, and Proposition~\ref{prop:disclosure} places one more
step beyond it: the rung certifies a number, and a claim is a number plus a rule
for reading it. The missing axis is disclosure of the per-trial record, and the
reason to insist on it is independent of anyone's honesty. The record examined
here declares its display rule in a field, names the median primary, and
annotates the mean as outlier-sensitive. Its authors did the careful thing. A
reader holding only the printed table still cannot check the conclusion, because
the declaration travels without the trials it was applied to.
...
less than the summary table that would sit beside them. On these numbers, cost
is a weak explanation for the absence of per-trial reporting.

\section{Scope of the results}

The contribution of this paper is to reporting practice, and it claims no
novelty in acoustic localisation. The estimators in the second and third studies
are local implementations written inside this project rather than anyone's
released code, and they are named by what they are given rather than by whose
method they resemble, so no comparison here is a finding about a published
method. Rung \NRungs{} is reported as unreachable and left unreached: the
required runtime was absent, no substitute runtime was treated as equivalent to
it, and no published table was reconstructed, approximated or synthesised.

The audit result is machine-relative and dated. It locates the first break at
rung \FirstBreak{} on one machine on one date and records the observation that
justifies it; installing the required runtime, or fetching the corpus, would
change the result, and the correct reading of the integer is a statement about
a machine on a day rather than a permanent property of the audited work. The
audit is a record, not an accusation: no misconduct or negligence is alleged,
and the quality of the audited work lies outside what this study evaluated.

The refined sweep is a new run made for this manuscript, and it is reported as
an extension of the audited record rather than as part of it or as a
confirmation of anything stated in advance. It leaves that record unchanged: the
estimator code is imported unmodified, and the sweep is stored as a separate
artifact beside the existing ones. It sits outside any pre-registered plan for
this study. What was fixed in advance is narrower and is stated where it is
used: the prediction the refined sweep is read against, and the tolerance for
calling it successful, were both computed and written down before the sweep ran.

The break-even results are statements about a design, and about these two
implementations in simulated rooms at a fixed prior width and noise floor. They
carry no recommendation for any array and no view on how anyone should have
designed a sweep: a grid placed where the modelled effect is expected to matter
is a reasonable grid, and the point is not that its authors erred but that the
sign it reports is not identified by the estimators alone. The prediction rule
is shown for one array, the one the coarse sweep ran; \CensoredRooms{} of the
arrays here have no measured crossing to test it against, so whether it would
locate another array's crossing is left open.

The mean is reported and kept in its place. The third comparison reports what
the mean says in order to show that it says something different from the trials
it summarises, which is an argument for disclosing the trials rather than for
choosing between summaries, so the mean is neither promoted to a display
quantity nor discarded. Every result reported here is computed from the full
sample; the trimming in Fig.~\ref{fig:influence} measures leverage and is
applied to no quantity stated as a finding.
...
for it and all \EntryPoints{} entry points are present, so availability is
complete; the first break is at rung \FirstBreak{}, the absence of the
commercial numerical environment the source requires, and the published table is
consequently out of reach on this machine. Corpus availability was tracked
separately and shows why it must be: the evaluation corpus is public, permanently
archived and about \CorpusGB{} gigabytes, and is absent locally, which is
neither a satisfied nor a broken requirement.
...
Not applicable. This study is entirely computational: it involves no human
participants, no animal subjects and no personal data. It records the
reachability of publicly released third-party research software on one machine
on one date, makes no assessment of the correctness or quality of that software
and alleges no wrongdoing.
...
The probe output, the derived tables, the recorded project state and every
per-trial record this manuscript reads, including the refined sweep and its
re-evaluation, accompany the manuscript source. Each artifact is recorded with
its SHA-256 checksum in a manifest, and the figure code verifies those checksums
against the bytes on disk before reading them, so the build stops rather than
silently reporting stale numbers. The evaluation corpus of the audited line of
work is a third-party public archive, cited in the text and not redistributed
here.
\input{generated_availability}

\subsection*{Code availability}

The read-only probe and the figure and table code that emit every number in
this manuscript are deposited with the data described above; the refined sweep
is deposited as its complete per-trial record. Code is released under the MIT
licence; the manuscript text, figures and recorded result data are released
under CC BY 4.0. The reference source examined in the audit is third-party
software, was not modified and is not redistributed.
...
from artifacts tied to the manuscript by cryptographic checksum rather than
transcribed into prose, and structural claims are enforced by build-time checks
that stop the build rather than by unverified sentences. The author reviewed and
edited all text and retains full responsibility for the content.
