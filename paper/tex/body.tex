...
recomputes each digest before reading, so a stale artifact stops the build
...
rung is therefore bound here alongside the measurements, and it is listed as
...
Two limits keep the table honest. A digest fixes bytes to a claim; it does not
...
The figure and table code recomputes each digest from the bytes on disk before
...
redistributed here. A public repository and persistent identifier will be
supplied after release review; neither is asserted here.
...
cryptographic digest rather than transcribed into prose. Structural claims are
