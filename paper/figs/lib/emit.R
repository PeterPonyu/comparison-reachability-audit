# Writing figures and the generated tex, and the text shaping the ladder needs.
# Nothing here decides anything; it only formats what the driver already read.
...
# The ladder sets whole sentences inside boxes of a fixed width, so the breaks
# are chosen here rather than left to the graphics device, which would clip.
...
# The paired tests here return either an ordinary p or one small enough that a
# fixed number of decimals would print it as zero, so the small ones are set in
# scientific notation instead of being rounded away.
p_cell <- function(p) {
  if (is.na(p)) return("---")
